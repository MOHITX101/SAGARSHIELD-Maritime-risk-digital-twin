...
### 1. Dynamic Risk Scoring (`agents/risk_agent.py`)
Ingests news streams using an automated Google News RSS parser or manual text input. Passes raw text payloads to Groq (`openai/gpt-oss-120b`) to generate structured JSON outputs containing localized threat scores and reasoning for key chokepoints. Includes automated fallback heuristics to ensure system continuity during network disruptions.

### 2. Linear Procurement Optimizer (`core/optimizer.py`)
Formulates supply distribution as a constrained Linear Program solved via `scipy.optimize.linprog`:

**Objective:**
Minimize Total Landed Cost = Σ [ Base Cost_i × (1 + Risk_i × 0.5) × Volume_i ]

**Constraints:**
* Total Volume = Import Need
* 0 ≤ Volume_i ≤ Base Capacity_i × (1 - Risk_i × 0.7)

*Risk Premiums:* As threat ratings rise, route landed costs increase due to insurance and fuel surcharges, while safe throughput capacities shrink.

### 3. Strategic Reserve Simulation Engine (`agents/reserve_agent.py`)
Monitors real-time import deficits (Unmet Demand = Import Need - Delivered Volume). Models daily extraction limits (1.2 MBD) against total reserve capacity (39.5 MB). Generates a daily 14-day depletion schedule and outputs visual burn-rate projections.
